...
router.post('/cashier-login', cashierPinLimiter, async (req, res) => {
  ...
  if (username || password || otp) {
    ...
    try {
      ...
      console.error('DB Cashier login error:', error.message);
      return res.status(500).json({ success: false, error: 'Unable to verify cashier credentials.' });
    }
  }
  ...
  try {
    ...
    console.error('DB Cashier PIN login error:', error.message);
    return res.status(500).json({ success: false, error: 'Unable to verify cashier PIN.' });
  }
});
